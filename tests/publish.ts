...
export async function main() {
  ...
  const publisher = Account.fromPrivateKey({ privateKey: new Ed25519PrivateKey(PUBLISHER_ACCOUNT_PK), legacy: false });
  await aptos.fundAccount({ accountAddress: publisher.accountAddress, amount: FUND_AMOUNT });
